...
$image_err = $firstName_err = $middleName_err = $lastName_err = $residentID_err = $birthday_err = $gender_err = $position_err = $email_err = $phoneNumber_err = $username_err = $password_err = "";
  ...
  if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sql3 = "SELECT purok from residentdata where id= $updateID";
    $result = $mysqli->query($sql3);

    if ($result->num_rows > 0) {
    // output data of each row
    while($row = $result->fetch_assoc()) {
        $previousPurok = $row["purok"];
    }
    }  
        ...
        $query_run = mysqli_query($mysqli, $sql);
            ...
            if ($query_run) {
                ...
                $sql4 = "SELECT purok from residentdata where purok = '$updatePurok'";
                $result2 = $mysqli->query($sql4);

                $total = mysqli_num_rows($result2);

                $sql5 = "UPDATE purok SET population = '$total' WHERE purok = '$updatePurok'";
                $query_run4 = mysqli_query($mysqli, $sql5);

                $sql6 = "SELECT purok from residentdata where purok = '$previousPurok'";
                $result3 = $mysqli->query($sql6);

                $total2 = mysqli_num_rows($result3);

                $sql7 = "UPDATE purok SET population = '$total2' WHERE purok = '$previousPurok'";
                $query_run5 = mysqli_query($mysqli, $sql7);
